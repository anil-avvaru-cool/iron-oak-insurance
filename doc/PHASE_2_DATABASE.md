...
def main():
    ...
    if args.truncate:
        with conn.cursor() as cur:
            cur.execute("TRUNCATE violations, telematics, claims, policies, customers, iso_claim_history CASCADE")
        conn.commit()
    ...
    violations       = json.loads((data / "violations.json").read_text())
    ...
    load_table(conn, "violations",       violations,        ["violation_id","customer_id","policy_number","state","violation_date","violation_type","points","conviction_date","expiry_date","source"])
...
docker exec iron-oak-insurance-postgres-1 psql -U aioi -d aioi -c "SELECT * FROM policies LIMIT 10;" > sqlOut.txt
docker exec iron-oak-insurance-postgres-1 psql -U aioi -d aioi -c "SELECT * FROM document_chunks WHERE source_type = 'policy_document' and customer_id is NOT NULL  LIMIT 10;" > sqlOut.txt
